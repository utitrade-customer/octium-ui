...
export const MarketsHotOnlist: React.FC<any> = () => {
	...
	const fetchMarketsKlines = async (marketId: string, from: number, to: number) => {
		try {
			const klines = await API.get({ apiVersion: 'peatio' })(
				`/public/markets/${marketId.split('/').join('')}/k-line?period=30&time_from=${from}&time_to=${to}`,
			);
				...
				<div className="container" onClick={() => handleRedirectToTrading(market ? market.id : '')}>
					...
					<div className="row mt-3" style={{ zIndex: 999, position: 'relative' }}>
						<div className="col-12 d-flex justify-content: center">
							<div>
								<span>{intl.formatMessage({ id: 'page.marketsLists.marketsHotOnList.volume' })}</span>
								<span className="ml-2" style={{ marginRight: '5px', fontWeight: 'bold' }}>
									{volume}
								</span>
