...
export const incrementDepthReducer = (state = initialIncrementDepth, action: DepthActions): DepthIncrementState => {
	switch (action.type) {
		...
		case DEPTH_DATA:
			const newState = { ...state, timestamp: action.payload.timestamp || Math.floor(Date.now() / 1000) };

			if (action.payload.asks) {
				newState.asks = handleIncrementalUpdateArray(state.asks, action.payload.asks as string[][], 'asks').slice(
					0,
					orderBookSideLimit(),
				);
			}
			...
			if (action.payload.bids) {
				newState.bids = handleIncrementalUpdateArray(state.bids, action.payload.bids as string[][], 'bids').slice(
					0,
					orderBookSideLimit(),
				);
			}
