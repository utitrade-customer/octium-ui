...
export type typeFCInputChange = (key: keyof VerifyAccount, value: string) => void;
...
export const PersonalVerify = (props: PersonalVerifyProps) => {
	...
	const inputChange: typeFCInputChange = (key, value) => {
		...
		switch (step) {
			case 1:
				...
				const isValidFullName =
					!xssRegex.test(verify.fullname) && verify.fullname.length > 0 && !REGEX_NUMBER.test(verify.fullname);
			...
			case 2:
				...
				const isValidPostalCity = !xssRegex.test(verify.city) && verify.city.length && !REGEX_NUMBER.test(verify.city);
				return !isValidAddress || !isValidPostalCode || !isValidPostalCity;
			case 3:
				const isValidDocumentNumber =
					!xssRegex.test(verify.document_number) &&
					verify.document_number.length &&
					REGEX_NUMBER.test(verify.document_number);
