...
export const BuyIEOComponent: React.FC<BuyIEOProps> = props => {
	...
	React.useEffect(() => {
		...
		setQuantityState(_toString(props.minBuy));
	}, [props.currencies[0]]);
	...
	const handleClickBuyAll = (checked: boolean) => {
		if (checked) {
			setTotalPriceState(_toString(quoteBalanceState));
		} else {
			setQuantityState(_toString(props.minBuy));
			const totalPrice = _toString(NP.strip(NP.times(props.minBuy, convertedPrice)));
	...
	const showBuyConfirmModalView = () => {
		...
		return check ? (
			<BuyConfirmModal
				...
				quoteTotal={_toNumber(totalPriceState)}
		...
		return (
			<div className="buy-ieo-desktop-close-view">
				<div className="buy-ieo-desktop-close-view__content">
					...
					<p>${convertEtoNumber(_toString(props.priceIEO))} USD</p>
	...
	React.useEffect(() => {
		if (Number(convertedPrice)) {
			...
			const totalPrice = _toString(NP.strip(NP.times(props.minBuy, convertedPrice)));
			const quantity = _toString(NP.strip(NP.divide(totalPrice, convertedPrice)));
			setTotalPriceState(totalPrice);
	...
	React.useEffect(() => {
		const quantity = _toString(NP.strip(NP.divide(totalPriceState, convertedPrice)));
		setQuantityState(quantity);
	}, [totalPriceState]);
		...
		<div id="buy-ieo-desktop" className="container">
			...
			>
						...
						<div className="text-right w-100">
							<div className="buy-ieo-desktop-container__balance__amount">
								<div className="avaiable">
									...
									<span>{_upperCase(props.currencyID)} </span>: {baseBalance}{' '}
									<span>{_upperCase(props.currencyID)}</span>
								</div>
							</div>
							<div className="buy-ieo-desktop-container__balance__amount">
				...
				<div className="buy-ieo-desktop-container__buy-form">
					...
					<div className="buy-ieo-desktop-container__buy-form__input-group d-flex justify-content-center">
						<div className="buy-ieo-desktop-container__buy-form__input-group__logo">
							<CurrencyIcon
								...
								currency_id={selectedCurrencyState}
						...
						<div className="buy-ieo-desktop-container__buy-form__input-group__input d-flex flex-wrap justify-content-center align-items-center">
							<div className="buy-all">
								<button
									...
									name="buy-all"
									type="button"
									value={_toString(btnBuyAllState)}
								>
									{!btnBuyAllState
							...
							<input
								type="text"
								value={formatNumber(convertEtoNumber(_toString(totalPriceState)))}
								placeholder="0"
								onChange={event => {
									const value = event.target.value.replace(/,/g, '');
									if (_toNumber(value) >= 0) setTotalPriceState(value);
								}}
							/>
						</div>
					...
					{_toNumber(totalPriceState) > Number(handleGetBalance(selectedCurrencyState)) ? (
						<span style={{ color: 'rgb(218 50 58)', fontWeight: 'bold', width: '100%' }}>
							{intl.formatMessage(
								{ id: 'page.ieo.detail.buyIeo.buyFrom.notEnough' },
								{ number: _toUpper(selectedCurrencyState) },
							)}
							{/* {`*** Not enough ${_toUpper(selectedCurrencyState)} balance to execute this order`} */}
						</span>
					) : null}
					<br />
					...
					<div className="buy-ieo-desktop-container__buy-form__input-group d-flex justify-content-center">
						...
						<input
							...
							value={formatNumber(convertEtoNumber(_toString(priceState)))}
							placeholder="0"
							style={{
								cursor: 'not-allowed',
							}}
					...
					<label htmlFor="quantityToBuy" className="mt-3">
						{intl.formatMessage(
							...
							{
								currency: _toUpper(props.currencyID),
							},
						)}
						{/* {` Estimated ${_toUpper(props.currencyID)} Quantity :`} */}
					</label>
					<div className="buy-ieo-desktop-container__buy-form__input-group d-flex" style={{ padding: '0' }}>
						...
						<input
							type="text"
							placeholder="0"
							value={formatNumber(_toString(Number(quantityState)))}
							className="buy-ieo-desktop-container__buy-form__input-group__input"
							name="quantityToBuy"
						/>
						<div className="buy-ieo-desktop-container__buy-form__input-group__denomination">
					...
					{props.minBuy > Number(quantityState) ? (
						<span style={{ color: 'rgb(218 50 58)', fontWeight: 'bold', width: '100%' }}>
							...
							{`${formatNumber(_toString(Number(props.minBuy)))} ${_upperCase(props.currencyID)}`}
						</span>
					) : (
						''
					)}
