...
export const OrderHistoryList: React.FC<OrderHistoryListProps> = ({ marketName }) => {
	...
	const dataRender = list.filter(item => item.market === market);
	return (
		<OrderHistoryListStyle>
			<TableOrder headersKeys={renderHeaders()} data={retrieveData(dataRender)} />
		</OrderHistoryListStyle>
	);
};
