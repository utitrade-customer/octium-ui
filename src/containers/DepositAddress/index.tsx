...
export const DepositAddress: React.FC<DepositAddressProps> = (props: DepositAddressProps) => {
	...
	const renderParentWallet = () => {
		const currency = currencies.find(cur => _toLower(cur.id) === _toLower(mainWallet.currency));
	...
	const renderNote = (currency: string) => {
		return (
			<div className="col-12 d-flex justify-content-between">
				<p className="pr-5">
					<strong>Send only {_toUpper(currency)} to this deposit address.</strong>
					<br />
					Sending coin or token other than {_toUpper(currency)} to this address may result in the loss of your deposit.
				</p>
			</div>
		);
	};

	const renderCoinDepositTabs = () => {
		return (
		...
		<div id="deposit-address">
			...
			>
				<div>
					...
					<div className="row">
						<div className="col-12">{renderCoinDepositTabs()}</div>
					</div>
				</div>
			</div>
		</div>
