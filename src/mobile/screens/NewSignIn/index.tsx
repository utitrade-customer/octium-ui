...
export const NewSignInMobileScreen: FC = () => {
	...
	const geetestCaptchaRef = React.useRef<any>(null);
	const setPasswordFocused = useState(false)[1];

	const handleReCaptchaSuccess: ReCAPTCHAProps['onChange'] = value => {
	...
	const handleGeetestCaptchaSuccess = (value: GeetestCaptchaResponse | undefined) => {
		setCaptcha(prevCaptcha => ({
			...prevCaptcha,
	...
	const renderCaptcha = () => {
		const { shouldGeetestReset } = captcha;
	...
	const canSubmit = (): boolean => {
		...
		const { reCaptchaSuccess, geetestCaptchaSuccess } = captcha;
