...
export const BuyIEOComponent: React.FC<BuyIEOProps> = props => {
	...
	const handleClickBuyAll = (checked: boolean) => {
		if (checked) {
			setTotalPriceState(_toString(handleGetBalance(selectedCurrencyState)));
		} else {
			setQuantityState(_toString(props.minBuy));
			setTotalPriceState(_toString(NP.strip(NP.times(props.minBuy, convertedPrice))));
		}
	...
	React.useEffect(() => {
		if (convertedPrice) {
			setQuantityState(_toString(NP.strip(NP.divide(totalPriceState, convertedPrice))));
		}
	}, [totalPriceState]);
	...
	React.useEffect(() => {
		if (convertedPrice) {
			...
			setTotalPriceState(_toString(NP.strip(NP.times(quantityState, convertedPrice))));
		}
	}, [convertedPrice]);
					...
					<div className="w-100">
						<div className="text-right w-100">
							<p className="buy-ieo-mobile-container__balance__amount">
								...
								{_upperCase(props.currencyID)}: <span>{baseBalance}</span> {`${_upperCase(props.currencyID)}`}
							</p>
							<p className="buy-ieo-mobile-container__balance__amount">
								{intl.formatMessage({ id: 'page.ieo.detail.buyIeo.balance.available' })}{' '}
