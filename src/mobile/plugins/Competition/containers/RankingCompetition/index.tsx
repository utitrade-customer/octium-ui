...
export const RankingCompetition: React.FC<RankingCompetitionProps> = props => {
	...
	return (
		<div
			id="ranking-mobile-competition-"
			className={`position-relative ${ranking.loading ? backgroundLoadingClassName : ''}`}
		>
			{ranking.loading ? <LoadingCompetition className="competition-loading col-12 position-absolute" /> : null}
			<div className="ranking-mobile-competition-__winner mt-2 mb-2 col-12 d-flex flex-column justify-content-center">
				<img width="60px" src={cup} alt="image-cup" className="m-auto ranking-mobile-competition-__winner__logo" />
				<div className="ranking-mobile-competition-__winner__uid m-auto">
					{ranking.payload.length ? ranking.payload[0].uid : ''}
				</div>
			</div>
			<div className="ranking-mobile-competition-__listing">
				<div className="ranking-mobile-competition-__listing__title d-flex">
					<div className="ranking-mobile-competition-__listing__title__cup">
					...
					<div className="ranking-mobile-competition-__listing__title__limit">
						{`Top ${!ranking.payload.length ? '' : ranking.payload.length} Rankings`}
					</div>
				</div>
				<div className="ranking-mobile-competition-__listing__content">
						...
						<tbody>
							{ranking.payload.map((item, index) => (
								<tr key={index} className="text-center">
									<td>{renderSerialRank(index + 1)}</td>
									<td>{item.uid}</td>
									<td>{item.volume}</td>
